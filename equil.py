...
def equilFromCrys(inputXML="mol.xml",\
                  outfile=None,\
                  steps=100000,\
                  rigid=True,\
                  potentials=None,\
    ...
    :param inputXML: File in which the initial configuration is stored in the hoomd-xml file format.
    :param outfile: File to output final configuration to.
    :param steps: Number of steps to run the equilibration.
    :param rigid: Boolean value indicating whether the rigid integrators should be used.
    :param potentials: Interaction potentials to use for the simulation. Default values are set if no input is given.
    :param temp: Target temperature at which to equilibrate system
    :param press: Target pressure for equilibration
    :param maxIters: Maximum number of iterations for convergence on target pressure and temperature.
    """
    ...
    if rigid:
        fire = integrate.mode_minimize_rigid_fire(group=gall, dt=0.005, ftol=1e-3, Etol=1e-4)
    else:
        fire = integrate.mode_minimize_fire(group=gall, dt=0.005, ftol=1e-3, Etol=1e-4)
    run(1000)
...
def equilFromFile(inputXml=None,\
                  outfile=None,\
                  temp=1.0,\
                  press=1.0,\
                  steps=100000,\
    ...
    :param inputXml: Input file containing input configuration in hoomd-xml format
    ...
    :param maxIters: Maximum number of iterations to run equilibration step if target temperature or pressure are not reached.
    ...
    # context.initialize()
    system = init.read_xml(filename=inputXml, restart=basename+'-restart.xml')
    update.enforce2d()
    ...
    # Calculate thermodynamic quantities
    thermo = analyze.log(filename=basename+"-thermo.dat", quantities=['temperature', 'pressure', 'potential_energy', 'rotational_kinetic_energy', 'translational_kinetic_energy'], period=1000)

    # Create restart file if simulation stops
    restart = dump.xml(filename=basename+'-restart.xml', period=1000000, restart=True, all=True)
