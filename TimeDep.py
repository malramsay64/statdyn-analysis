...
class TimeDep2dRigid(TimeDep):
    ...
    def print_data(self, system, outfile='out.dat'):
        """Print rotations and translations to file

        This function outputs the translation and rotation of each molecule
        to a file allowing for later analysis. The data is printed to file
        in a JOSN format with all data from each interval in separate objects
        each containing a small amount of metadata.

        Args:
            system (system): The hoomd system objet
            outfile (string): The filename to write the data to
