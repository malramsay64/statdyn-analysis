...
alpha <- p + geom_path(aes(y=alpha)) + labs(y=expression(frac(symbol("\341") *Delta *r^4 *symbol("\361"), 2*symbol("\341")* Delta *r^2 *symbol("\361")^2) -1))
...
gamma1 <- p + geom_path(aes(y=gamma1)) + labs(y=g1)
